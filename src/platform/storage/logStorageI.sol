...
interface LogStorageI { 
  ...
  //App events 
  function addAppEvent(uint _app, uint32 _hashType, uint32 _appType, uint32 _store, uint _price, bool _publish, address _dev, string _hash) external;
  ...
  function changePublishEvent(uint _app, bool publish) external;
  function changePriceEvent(uint _app, uint price) external;

  function buyAppEvent(address user, address developer, uint _app, address adrNode, uint price) external;
  ...
  function setConfirmationDevEvent(address _dev, bool _state) external;
  ...
  function changeHashICOEvent(uint _app, string _hash, uint32 _hashType) external;
